...
export default function UserChange({
  ...
  function handleSubmit({ dni, name, lastName, email, password, fecha_nacimiento, direccion,telefono,isVacuna, isAdmin, active }) {
    const newUser = {
      id: user.id,
      dni,
      name,
    ...
    <Dialog open onClose={() => onCancel(false)}>
      ...
      <Content>
        <Form initialData={user} schema={schema} onSubmit={handleSubmit}>
          <Divider />
          <Label>Vacunacion</Label>
          <Radio
            name="type"
            value="No Vacunado"
            label="No Vacunado"
            disabled={allowModify}
            defaultChecked={typeUser === 'No Vacunado'}
          ...
          <Radio
            ...
            value="Vacunado"
            label="Vacunado"
            defaultChecked={typeUser === 'Vacunado'}
            onChange={e => setTypeUser(e.target.value)}
          />
          <Divider />
