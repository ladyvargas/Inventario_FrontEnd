...
function isRegisteredDNI({ dni }) {
  const database = JSON.parse(fs.readFileSync(databaseDir, "UTF-8"));

  return database.users.some(user => user.dni === dni);
}
...
server.post("/auth/register", (req, res) => {
  const { dni, name, lastName, email, password, fecha_nacimiento, direccion,telefono,isVacuna, isAdmin, active } = req.body;

  if (isRegistered({ email })) {
    return res
      .status(400)
      .json({ message: "Correo electrónico ya registrado en la base de datos" });
  }
  if (isRegisteredDNI({ dni })) {
    return res
      .status(400)
      .json({ message: "DNI ya registrado en la base de datos" });
  }
...
server.use(/^(?!\/auth).*$/, (req, res, next) => {
  if (
    ...
    return res.status(401).json({ message: "Formato de ficha no válido" });
  }
  try {
